...
procedure leerDetalle (var det : archivoDetalles; var rDet : rDetalle); 
begin
    if (not eof (det) ) then 
        read(det,rDet)
    else det.fecha:= valorFechaAlto; 
end; 

procedure detalleMin (var vecDet : vecDetalles; var vecRegDet : vecRegDetalles; var min : rDetalle);
var
    i,posMin : integer;
begin
    ...
    for i:= 1 to cantDetalles do 
        ...
        if (min.fecha <> valorAlto) then 
            read(vecDetalles[posMin],vecRegDet[posMin]); 
end; 
    ...
    var
        ...
        rMin,rMax : rDetalle; 
    begin
        actualizarMaestro(rMaestro,vecDet,rMin,rMax); 
    end.
