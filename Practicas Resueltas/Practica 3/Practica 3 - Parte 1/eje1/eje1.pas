...
type
	...
	* 	Si se encuentra ese numero de empleado ==> Mostramos un mensaje diciendo que nro de empleado ya existe.}
	...
	//-------------------------------------------------------------------------------------------------------------
	// Opcion 8
	procedure leer (var arch : archivoEmpleados; var e : empleado); 
	begin
	...
	function buscarEmpleado (var arch: archivoEmpleados; nroBuscado : integer): integer; 
	var 
		e : empleado; 
		pos : integer; 
	begin
		reset(arch);
		leer(arch,e);  
		{buscamos la posicion del nroEmpleado}
		while ( ( e.nro <>valorAlto) and (e.nro <> nroBuscado) ) do 
			leer(arch,e);
			
		 if (e.nro = nroBuscado) then pos:= filePos(arch)-1
		else  pos:= -1; 
		close(arch); 
		buscarEmpleado := pos;
	end; 
	
	procedure eliminarAux (var arch : archivoEmpleados; pos : integer); 
	var
		emp : empleado; 
	begin
		reset(arch); 
		seek(arch,fileSize(arch)-1);  // => Nos paramos en el ultimo registro del archivo
		read(arch,emp); // => Leemos el ultimo registro del archivo
		
		seek(arch,pos);  // => Ponemos el puntero en la posicion que queremos eliminar 
		write(arch,emp); // => Escribimos el ultimo registro en la posicion a eliminar.
	...
	var
		pos,nroEmpleado: integer; 
	begin
		write ('Ingrese un numero de empleado para eliminar: '); readln (nroEmpleado); 
		pos:= buscarEmpleado(arch,nroEmpleado); 
		if (pos <> -1) then eliminarAux(arch,pos)
	...
	begin
		...
		writeln ('Ingrese 8 si: Desea eliminar un empleado del archivo.'); 
